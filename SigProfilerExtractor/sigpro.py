...
def record_parameters(sysdata, execution_parameters, start_time):
    ...
    ###########################################
    ######## Modify By Jian 2022-10-27 ########
    ###########################################
    #sysdata.write("\tcosmic_version: {}\n".format(execution_parameters["cosmic_version"]))
    sysdata.write("\tSignature_Database: {}\n".format(execution_parameters["Signature_Database"]))
...
def sigProfilerExtractor(input_type, 
    ...
    for m in mtypes:
        ...
        #decomp.spa_analyze(allgenomes, output, signatures=processAvg, genome_build=genome_build, signature_database=cosmic_version, exome=exome, verbose=False,decompose_fit_option= True,denovo_refit_option=True,cosmic_fit_option=False,devopts=devopts)
        decomp.spa_analyze(allgenomes, output, signatures=processAvg, genome_build=genome_build, signature_database=Signature_Database, exome=exome, verbose=False,decompose_fit_option= True,denovo_refit_option=True,cosmic_fit_option=False,devopts=devopts)
